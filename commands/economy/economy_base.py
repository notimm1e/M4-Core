...
def apply_loss(user_id, data, amount):
    """Subtract amount from wallet; overflow goes to debt."""
    uid = str(user_id)
...
def apply_earnings(user_id, data, amount):
    """Apply earnings — clears debt first, remainder goes to wallet. Returns (debt_paid, to_wallet)."""
    uid = str(user_id)
    debt = data[uid]["debt"]
    if debt > 0:
        if amount >= debt:
...
async def debt_prompt(ctx, bot, data, user_id):
    """
    Show debt prompt if user is in debt.
    Returns updated data (with any bank payment applied).
    Returns None if the user timed out (caller should abort).
    """
    uid = str(user_id)
    debt = data[uid]["debt"]
    if debt == 0:
