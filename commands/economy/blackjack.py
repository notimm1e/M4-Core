...
class Blackjack(commands.Cog):
    ...
    async def blackjack(self, ctx, amount: int):
        ...
        def create_embed(show_dealer=False):
            ...
            embed.add_field(name="◈ dealer's hand", value=f"total: `{d_score}`")
            if not show_dealer:
        ...
        if res_type == "loss":
            ...
            if debt > 0:
                result += f"\n⌬ {debt:,} now in debt"
        elif res_type == "win" and 'debt_paid' in locals() and debt_paid: # type: ignore
        ...
        if res_type == "win":
            final_embed.color = 0x57f287
        elif res_type == "loss":
            final_embed.color = 0xed4245
        else:
            final_embed.color = 0xfee75c

        if ai_msg:
            final_embed.add_field(name="slug", value=ai_msg, inline=False)
